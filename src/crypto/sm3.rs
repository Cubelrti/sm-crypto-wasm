...
pub fn sm3_hash_u32(msg: &[u8]) -> Vec<u32> {
    let iv: Vec<u32> = vec![
...
pub fn sm3_hmac(key: &[u8], msg: &[u8]) -> Vec<u8> {
    ...
    let mut i_key_pad_msg = i_key_pad;
    i_key_pad_msg.extend_from_slice(msg);
    let i_key_pad_msg_hash = sm3_hash_u32(&i_key_pad_msg)
        .iter()
        .flat_map(|x| x.to_be_bytes())
        .collect::<Vec<u8>>();

    let mut o_key_pad_i_key_pad_msg_hash = o_key_pad;
    o_key_pad_i_key_pad_msg_hash.extend_from_slice(&i_key_pad_msg_hash);

    sm3_hash_u32(&o_key_pad_i_key_pad_msg_hash)
        .iter()
        .flat_map(|x| x.to_be_bytes())
        .collect::<Vec<u8>>()
}
